...
func WriteBundle(outputDir, sourceDir, repository, tag string) (Bundle, error) {
	...
	err := filepath.WalkDir(sourceDir, func(path string, entry fs.DirEntry, err error) error {
		...
		if err := validateNamespace(path, manifest); err != nil {
			return err
		}
	...
	for _, app := range bundle.Apps {
		if err := writeNamespace(app.Dir, app.Name); err != nil {
			return Bundle{}, err
		}
	}
...
func validateNamespace(path string, manifest *unstructured.Unstructured) error {
	if manifest.GetKind() == namespaceKind {
		return fmt.Errorf("%s: Namespace manifests are generated by toolbox", path)
	}

	if manifest.GetNamespace() != "" {
		return fmt.Errorf("%s: %s/%s metadata.namespace must be omitted; Flux sets it from the app path", path, manifest.GetKind(), manifest.GetName())
	}
...
func writeNamespace(outputDir, name string) error {
	...
	return writeManifest(filepath.Join(outputDir, "namespace.yaml"), manifest)
}
...
func writeRootBundle(outputDir string, apps []AppBundle, tag string) error {
	...
	for _, app := range apps {
		...
		for _, manifest := range manifests {
			...
			if err := writeManifest(filepath.Join(outputDir, filename), manifest); err != nil {
				return err
			}
		}
	}
