...
`
	...
	writeFile(t, filepath.Join(sourceDir, "khuedoan", "blog", "staging", "service-blog.yaml"), manifest)
	...
	copied, err := os.ReadFile(filepath.Join(app.Dir, "service-blog.yaml"))
	if err != nil {
	...
	if !strings.Contains(namespace, "name: khuedoan-blog-staging") ||
		!strings.Contains(namespace, "istio.io/dataplane-mode: ambient") {
		t.Fatalf("generated namespace has wrong content:\n%s", namespace)
	}
	...
	if !strings.Contains(kustomization, "path: .") ||
		!strings.Contains(kustomization, "name: khuedoan-blog-staging") ||
		!strings.Contains(kustomization, "targetNamespace: khuedoan-blog-staging") {
...
func TestWriteBundleRejectsInvalidInput(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			path:    "khuedoan/blog/staging/service-blog.yaml",
			content: "controllers: {}\n",
...
`,
		...
		},
		{
			...
			path: "khuedoan/blog/staging/namespace.yaml",
			content: `apiVersion: v1
kind: Namespace
metadata:
  name: khuedoan-blog-staging
`,
			wantErr: "Namespace manifests are generated by toolbox",
		},
		{
...
metadata:
  ...
  namespace: other
`,
			wantErr: "metadata.namespace must be omitted",
		},
		{
